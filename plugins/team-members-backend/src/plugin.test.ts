...
import { teamMembersPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [teamMembersPlugin],
    });

    await request(server).get('/api/team-members/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/team-members/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/team-members/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/team-members/todos/${createdTodoItem.id}`)
